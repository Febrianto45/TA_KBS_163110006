  ...
  <div class="container-fluid" id="content-pencarian">
    <div class="container">
      ...
      ';
              ...
              $queryCari = "SELECT * FROM tabel_barang WHERE namaBarang LIKE '%$keyword%'";
              ...
              if ($jumlahCari > 0) {
                while ($arrayCari = mysqli_fetch_array($query_cari)) {
                  echo '
            <li>
        <a href="#' . $arrayCari['idBarang'] . '">
          <img src="admin/proses/' . $arrayCari['path'] . '" alt="' . $arrayCari['namaBarang'] . '">
          <span></span>
        </a>
        <div class="overlay" id="' . $arrayCari['idBarang'] . '">
          <a href="#" class="close"><i class="glyphicon glyphicon-remove"></i></a>
          <img src="admin/proses/' . $arrayCari['path'] . '">
          <div class="keterangan">
            <div class="container">
              <h4><strong>' . $arrayCari['namaBarang'] . '</strong></h4>
              <p>' . $arrayCari['keterangan'] . '</p>
              <h5>Rp.' . $arrayCari['harga'] . '</h5>
              <h5 class="kg">Satuan : ' . $arrayCari['satuan'] . '</h5>
              <button type="button" class="btn btn-success">Stock : ' . $arrayCari['stock'] . '</button>
              ';
                  if (isset($_SESSION['idUser'])) {
                    echo '
                  <a href="proses/beli.php?idBarang=' . $arrayCari['idBarang'] . '&idUser=' . $iduser . '"><button type="button" class="btn btn-info">Masukkan Keranjang</button></a>
                ';
                  } else {
                    echo '
